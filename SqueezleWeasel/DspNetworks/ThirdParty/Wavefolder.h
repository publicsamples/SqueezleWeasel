...
#include "src/Wavefolder.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct WavefolderMetaData {
		SN_NODE_ID("Wavefolder");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using Wavefolder = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _Wavefolder, WavefolderMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
